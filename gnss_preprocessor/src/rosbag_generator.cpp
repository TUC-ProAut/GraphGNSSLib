...
class rosbag_generator
{
    ...
    std::map<int, gnss_msgs::GNSS_Raw_Array> gnss_raw_map;
    std::map<int, nav_msgs::Odometry> doppler_map;
    std::map<int, gnss_msgs::GNSS_Raw_Array> station_gnss_raw_map;

    GNSS_Tools m_GNSS_Tools; // utilities  

    /* subscriber */
    std::unique_ptr<message_filters::Subscriber<gnss_msgs::GNSS_Raw_Array>> gnss_raw_array_sub;
    std::unique_ptr<message_filters::Subscriber<nav_msgs::Odometry>> doppler_sub;
    std::unique_ptr<message_filters::Subscriber<gnss_msgs::GNSS_Raw_Array>> station_gnss_raw_array_sub;
    std::unique_ptr<message_filters::TimeSynchronizer<gnss_msgs::GNSS_Raw_Array, gnss_msgs::GNSS_Raw_Array, nav_msgs::Odometry>> syncdoppler2GNSSRaw;
...
public:
    ...
    {      
        ...
        gnss_raw_array_sub.reset(new message_filters::Subscriber<gnss_msgs::GNSS_Raw_Array>(nh, "/gnss_preprocessor_node/GNSSPsrCarRov1", 10000));
        ...
        station_gnss_raw_array_sub.reset(new message_filters::Subscriber<gnss_msgs::GNSS_Raw_Array>(nh, "/gnss_preprocessor_node/GNSSPsrCarStation1", 10000)); // measurements from station
        syncdoppler2GNSSRaw.reset(new message_filters::TimeSynchronizer<gnss_msgs::GNSS_Raw_Array, gnss_msgs::GNSS_Raw_Array, nav_msgs::Odometry>(*gnss_raw_array_sub, *station_gnss_raw_array_sub, *doppler_sub, 32));
        ...
        pubGNSSRaw = nh.advertise<gnss_msgs::GNSS_Raw_Array>("/rosbag_generator_node/GNSSPsrCarRov1", 100); //

        /* publish the raw gnss data from station*/ 
        pubStationGNSSRaw = nh.advertise<gnss_msgs::GNSS_Raw_Array>("/rosbag_generator_node/GNSSPsrCarStation1", 100); // 
    ...
    */
    void span_bp_callback(const novatel_oem7_msgs::INSPVAXConstPtr& fix_msg)
    {
        ...
        if(finishGNSSReader)
        {
            std::map<int, gnss_msgs::GNSS_Raw_Array>::iterator GNSSiter_pr;
            GNSSiter_pr = gnss_raw_map.begin();
            int length = gnss_raw_map.size();

            /* find rover gnss data and publish */
            gnss_msgs::GNSS_Raw_Array roverGNSSRawData;
            ...
            gnss_msgs::GNSS_Raw_Array stationGNSSRawData;
   ...
   * @return void
   @ 
   */
   void gnssraw_doppler_msg_callback(const gnss_msgs::GNSS_Raw_ArrayConstPtr& gnss_msg, const gnss_msgs::GNSS_Raw_ArrayConstPtr& station_gnss_msg, const nav_msgs::OdometryConstPtr& doppler_msg)
    {
        m_gnss_raw_mux.lock();
