...
public:
    ...
    std::map<double, gnss_msgs::GNSS_Raw_Array> gnss_raw_map;
    std::map<double, nav_msgs::Odometry> doppler_map;
    std::map<double, gnss_msgs::GNSS_Raw_Array> station_gnss_raw_map;
...
public:
    ...
    bool input_gnss_raw_data(gnss_msgs::GNSS_Raw_Array GNSS_data, double timestamp)
    {
    ...
    bool input_station_data(gnss_msgs::GNSS_Raw_Array GNSS_data, double timestamp)
    {
        if(timestamp<0) return false;
        else 
        {
    ...
    */
        ...
        for(int k = 0;  k < length; k++,iter_sm++) // tranverse the whole station gnss measurements map
        {
            ...
            */
            ...
            for(int q = 0; q < sv_cnt; q++)
            {
                ...
                */
                gnss_msgs::GNSS_Raw u_master_sv, u_iSV, r_master_sv;
    ...
    {
        std::map<double, gnss_msgs::GNSS_Raw_Array>::iterator iter_pr;
        ...
        {
            gnss_msgs::GNSS_Raw_Array gnss_data = (iter_pr->second);
    ...
    {
        int length = measSize;
        std::map<double, gnss_msgs::GNSS_Raw_Array>::iterator iter;
        iter = gnss_raw_map.begin();
        for(int i = 0; i <length; i++,iter++)
        {
            if(i>=(lastFactorGraphSize-1))
            {
                gnss_msgs::GNSS_Raw_Array gnss_data = (iter->second);
    ...
    bool solveAmbiguityResolutionViaCovPropogation()
    {
        ...
        std::map<double, gnss_msgs::GNSS_Raw_Array>::iterator iter_cov; // station gnss measurements map iterator
        iter_cov = station_gnss_raw_map.end();
        iter_cov--;

        gnss_msgs::GNSS_Raw_Array st_gnss_data = (iter_cov->second);
        int sv_cnt = st_gnss_data.GNSS_Raws.size();
