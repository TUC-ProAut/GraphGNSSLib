...
struct DDMeasurement
{
    ...
    gnss_msgs::GNSS_Raw u_master_SV;
    gnss_msgs::GNSS_Raw u_iSV;

    gnss_msgs::GNSS_Raw r_master_SV;
    gnss_msgs::GNSS_Raw r_iSV;
...
Eigen::Vector3d getXYZ_error(Eigen::MatrixXd est, Eigen::MatrixXd refSat, std::string format)
{
 ...
 Eigen::MatrixXd getAllPositions(gnss_msgs::GNSS_Raw_Array GNSS_data)
{
...
Eigen::MatrixXd getAllMeasurements(gnss_msgs::GNSS_Raw_Array GNSS_data)
{
...
output: bool 
*/
bool checkAvailability(gnss_msgs::GNSS_Raw_Array GNSS_data) //
{
  bool result = true;
...
int getBeiDouCnt(gnss_msgs::GNSS_Raw_Array GNSS_data)
{
  int cnt = 0; 
...
int getGPSCnt(gnss_msgs::GNSS_Raw_Array GNSS_data)
{
  int cnt = 0; 
  for(int i =0; i < GNSS_data.GNSS_Raws.size(); i++)
  {
...
bool checkRepeating(gnss_msgs::GNSS_Raw_Array GNSS_data) 
{
  std::vector<int> satList;
  gnss_msgs::GNSS_Raw_Array GNSS_dataTmp;
  satList.clear();
  for(int i =0; i < GNSS_data.GNSS_Raws.size(); i++) // 
  {
...
void removeRepeatedSV(gnss_msgs::GNSS_Raw_Array &GNSS_data) 
{
  std::vector<int> satList;
  gnss_msgs::GNSS_Raw_Array GNSS_dataTmp;
  satList.clear();
...
/**
  ...
  */
   ...
   * @return eWLSSolution 5 unknowns with two clock bias variables
   @ 
  */
  Eigen::MatrixXd WeightedLeastSquare(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, gnss_msgs::GNSS_Raw_Array GNSS_data, std::string method){
  ...
  Eigen::MatrixXd getPseudorangeResidual(Eigen::MatrixXd eWLSSolution, gnss_msgs::GNSS_Raw_Array GNSS_data)
  {
...
/**
   ...
   * @param gnss_msgs::GNSS_Raw_Array GNSS_data
   * @return weight_matrix
   @ 
   */
  Eigen::MatrixXd cofactorMatrixCal_WLS(gnss_msgs::GNSS_Raw_Array GNSS_data, std::string method)
  {
   ...
   * @return weight_matrix
   @ 
   */
  Eigen::MatrixXd cofactorMatrixCal_R_WLS(gnss_msgs::GNSS_Raw_Array GNSS_data, std::string method)
  {
    Eigen::Matrix<double,4,1> parameters;
    double factor = 0.75;
...
/**
   ...
   * @return eWLSSolution 5 unknowns with two clock bias variables
   @ 
  */
  Eigen::MatrixXd WeightedLeastSquare_GPS(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, gnss_msgs::GNSS_Raw_Array GNSS_data){
   ...
   * @return covariance matrix for the estimated states
   @ 
  */
  Eigen::MatrixXd getCovarianceMatrix(Eigen::MatrixXd eAllSVPositions, Eigen::MatrixXd eAllMeasurement, gnss_msgs::GNSS_Raw_Array GNSS_data, std::string method){
  ...
  */
  ...
  double getVarofCp_ele_SNR(gnss_msgs::GNSS_Raw single_sat_data)
  {
    Eigen::Matrix<double,4,1> parameters;
    parameters<<50.0, 30.0, 30.0, 10.0; // loosely coupled 
    // parameters<<50.0, 30.0, 20.0, 30.0; // loosely coupled 
  ...
  double getVarofpr_ele_SNR(gnss_msgs::GNSS_Raw single_sat_data)
  {
    Eigen::Matrix<double,4,1> parameters;
    parameters<<50.0, 30.0, 30.0, 10.0; // loosely coupled 
    // parameters<<50.0, 30.0, 20.0, 30.0; // loosely coupled 
